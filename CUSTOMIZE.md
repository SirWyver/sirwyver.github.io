...
You can add extra information to a publication, like a PDF file in the `assets/pdfs/` directory and add the path to the PDF file in the BibTeX entry with the `pdf` field. Some of the supported fields are: `abstract`, `altmetric`, `arxiv`, `bibtex_show`, `blog`, `code`, `dataset`, `dimensions`, `doi`, `eprint`, `html`, `isbn`, `pdf`, `pmid`, `poster`, `slides`, `supp`, `video`, and `website`.

Use `dataset` for a dataset landing-page URL. The `code` link is labeled **Code** by default; set `code_label` when a more specific label is needed, such as **Dataset code**. These display fields and preview dimensions are excluded from exported citations.
